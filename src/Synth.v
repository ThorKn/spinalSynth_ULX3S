...
// Component : Synth
// Git hash  : 2df202774bbe0125a24e533a1f109c67b687cf58
...
module Synth (
  ...
  wire       [2:0]    core_oscillator_io_waveSelect;
  ...
  Oscillator core_oscillator (
    ...
    .io_freqWord   (core_registerBank_io_oscFrequency[23:0]), //i
    .io_waveSelect (core_oscillator_io_waveSelect[2:0]     ), //i
    .io_pwmWidth   (core_registerBank_io_oscPulseWidth[7:0]), //i
  ...
  assign core_oscillator_io_waveSelect = core_registerBank_io_oscWaveform[2 : 0];
...
module UartRx (
  ...
  wire                when_UartRx_l86;
  wire                when_UartRx_l103;
  wire                when_UartRx_l110;
  wire                when_UartRx_l128;
  wire                when_UartRx_l133;
  `ifndef SYNTHESIS
  ...
  assign when_UartRx_l70 = (io_rx == 1'b0);
  assign when_UartRx_l81 = (baudCounter == 8'h67);
  assign when_UartRx_l86 = (io_rx == 1'b0);
  assign when_UartRx_l103 = (baudCounter == 8'hcf);
  assign when_UartRx_l110 = (bitCounter == 3'b111);
  assign when_UartRx_l128 = (baudCounter == 8'hcf);
  assign when_UartRx_l133 = (io_rx == 1'b1);
  always @(posedge io_clk24MHz or posedge io_reset) begin
    if(io_reset) begin
      state_2 <= State_IDLE;
      baudCounter <= 8'h0;
      ...
      case(state_2)
        ...
        State_DATA : begin
          if(when_UartRx_l103) begin
            baudCounter <= 8'h0;
            shiftReg[bitCounter] <= io_rx;
            if(when_UartRx_l110) begin
              bitCounter <= 3'b000;
              state_2 <= State_STOP;
            end else begin
        ...
        State_STOP : begin
          if(when_UartRx_l128) begin
            baudCounter <= 8'h0;
            if(when_UartRx_l133) begin
              dataReg <= shiftReg;
              state_2 <= State_DONE;
            end else begin
              state_2 <= State_IDLE;
